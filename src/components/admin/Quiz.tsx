...
export default function Admin() {
  const { state, nextQuestion, prevQuestion, navigate } = useSocketContext()
  ...
  return <div>
    ...
    <div className="flex justify-between">
      ...
      <div>
        ...
        {state.questionIndex === state.questions?.length - 1 && <Link to='/admin/results' onClick={()=>navigate?.('/results')} className="text-white font-bold rounded bg-green-500 px-4 py-1">Show Results</Link>}
      </div>
    </div>
  </div >
}
